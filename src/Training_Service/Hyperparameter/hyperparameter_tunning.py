...
class hyperparameter:
    ...
    def compute_xgboost_hyperparameters(self,x_train,y_train,x_test,y_test,parameters_grid,cv_grid,verbose_grid,parameters_random,cv_random,verbose_random):
        ...
        try:
            grid.fit(x_train.drop(['cluster'],axis=1).to_numpy(),y_train.to_numpy()) # fitting the GridSearchCV with x_train and y_train
        ...
        y_pred=grid.predict(x_test.drop(['cluster'],axis=1).to_numpy()) # predict the x_test
        ...
        try:
            random.fit(x_train.drop(['cluster'],axis=1).to_numpy(),y_train.to_numpy()) # fitting the RandomizedSearchCV with x_train and y_train
        ...
        y_pred=random.predict(x_test.drop(['cluster'],axis=1).to_numpy()) # predict the x_test
