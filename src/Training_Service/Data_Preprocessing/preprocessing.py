...
class process_data:
    def __init__(self):
        self.log=logger.log()
    def create_csv_to_dataframe(self,path):
        try:
            df= pd.read_csv(path)
            self.log.log_writer(f'Successfully converted csv file from path {path} to Pandas DataFrame','preprocessing.log')
        ...
        return df
    def drop_unnecessery_columns(self,df,columns_to_remove):
        try:
            df=self.create_csv_to_dataframe('Master_Training_File/Zomato.csv')
            df.drop(columns_to_remove,axis=1,inplace=True)
            self.log.log_writer(f'Successfully removed the columns {columns_to_remove} from Pandas DataFrame','preprocessing.log')
        except Exception as e:
            self.log.log_writer(f'Couldnnot removed the columns {columns_to_remove} from Pandas DataFrame error: {str(e)}','preprocessing.log')
        return df
    def rename_the_columns(self,df,dictionary):
        try:
            ...
            self.log.log_writer(f'Successfully renames the columns from {dictionary.keys()} to {dictionary.values()} in Pandas DataFrame','preprocessing.log')
        except Exception as e:
            self.log.log_writer(f'Could not renames the columns from {dictionary.keys()} to {dictionary.values()} in Pandas DataFrame error: {str(e)}','preprocessing.log','Error')
        return df
    ...
    def drop_nan_values(self,df):
        ...
        if null_values>0:
            try:
                ...
                self.log.log_writer(f'There is {null_values} nan values present in this dataset that are sucessfully removed','preprocessing.log',message_type='Warning')
            except Exception as e:
                self.log.log_writer(f'There is {null_values} nan values present in this dataset that are not removed due to an error: {str(e)}','preprocessing.log',message_type='Error')
        else:
            self.log.log_writer('There is no nan values in this dataset','preprocessing.log',message_type='Warning')
        return df
    def cleaning_the_data_present_in_the_features(self,df):
        try:
            ...
            self.log.log_writer(f'Sucessfuly cleaned and change the datatype of feature rate','preprocessing.log',message_type='INFO')
        except Exception as e:
            self.log.log_writer(f'Could not cleaned and change the datatype of feature rate due to error: {str(e)}','preprocessing.log',message_type='Error')
        try:
            ...
            self.log.log_writer(f'Sucessfuly cleaned and change the datatype of feature cost','preprocessing.log',message_type='INFO')
        except Exception as e:
            self.log.log_writer(f'Could not cleaned and change the datatype of feature cost due to error {str(e)}','preprocessing.log',message_type='Error')
        return df
    def create_dummy_columns(self,df,columns,drop_first=True):
        try:
            dummy=pd.get_dummies(df,columns=columns,drop_first=drop_first)
            self.log.log_writer(f'Sucessfully created the dummy variables of columns {columns} now the column size is increased from {df.shape[1]} to {dummy.shape[1]}','preprocessing.log',message_type='INFO')
        except Exception as e:
            self.log.log_writer(f'Could not create the dummy variables of columns {columns} due to error: {str(e)}','preprocessing.log',message_type='Error')
        return dummy
