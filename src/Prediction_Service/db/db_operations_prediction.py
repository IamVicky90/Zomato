...
class db_ops:
    ...
    def con_establish(self,dbname):
        self.log.log_writer(f'Establishing Connection with {dbname} ....','DB_Operations.log',service_name='p')
    ...
    def create_Table(self,Table_name,dbname):
        if dbname in os.listdir(os.getcwd()):
            ...
            self.log.log_writer(f'DataBase {dbname} removed','DB_Operations.log','Warning',service_name='p')
        try:
            conn=self.con_establish(dbname)
            self.log.log_writer(f'Successfully Established Connection with {dbname}','DB_Operations.log',service_name='p')
        except Exception as e:
            self.log.log_writer(f'Could not Established Connection with {dbname} error: {str(e)}','DB_Operations.log','Error',service_name='p')
        cur=conn.cursor()
        i=0
        try:
            for col_name,d_type in self.json_file['ColName'].items():
                ...
                try:
                    ...
                    self.log.log_writer(f'Could not create the table or column error: {str(e)}','DB_Operations.log','Error',service_name='p')
            
            self.log.log_writer(f"Successfully created the {Table_name} along with it's columns",'DB_Operations.log',service_name='p')
        except Exception as e:
                self.log.log_writer(f'Could not create the table or column error: {str(e)}','DB_Operations.log','Error',service_name='p')
        ...
        try:
            ...
            for file in os.listdir('Prediction_Good_Data_Folder'):
                with open(os.path.join('Prediction_Good_Data_Folder',file),) as csvfile:
                    ...
                    conn.commit()
            self.log.log_writer(f'Sucessfully inserted the values into table {table}','DB_Operations.log',service_name='p')
        except Exception as e:
            self.log.log_writer(f'Could not insert values into table {table} error: {str(e)}','DB_Operations.log','Error',service_name='p')
    def dump_data_from_database_to_one_csv_file(self,dbname,table): #To create a master file of this sqllite database
        try:
            ...
            csv_file.to_csv(os.path.join('Master_Prediction_File','Zomato_prediction.csv'),index=False)
            self.log.log_writer(f'Successfully dump the data from {dbname} database to one csv file','DB_Operations.log',service_name='p')
        except Exception as e:
            self.log.log_writer(f'Could not  dump the data from {dbname} database to one csv file error: {str(e)}','DB_Operations.log','Error',service_name='p')
